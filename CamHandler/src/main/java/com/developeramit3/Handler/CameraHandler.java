package com.developeramit3.Handler;
...
import com.developeramit3.adapters.InstantImageAdapter;
import com.developeramit3.adapters.MainImageAdapter;
import com.developeramit3.interfaces.OnSelectionListener;
import com.developeramit3.interfaces.WorkFinish;
import com.developeramit3.modals.Img;
import com.developeramit3.utility.Constants;
import com.developeramit3.utility.HeaderItemDecoration;
import com.developeramit3.utility.ImageVideoFetcher;
import com.developeramit3.utility.PermUtil;
import com.developeramit3.utility.Utility;
import com.developeramit3.utility.ui.FastScrollStateChangeListener;
...
*/
public class CameraHandler extends AppCompatActivity implements View.OnTouchListener {
                    ...
                    case RecyclerView.SCROLL_STATE_DRAGGING:
                        ...
                        if (mScrollbar.getVisibility() != View.VISIBLE) {
                            ...
                            if (!Utility.isViewVisible(mScrollbar) && (recyclerView.computeVerticalScrollRange()
                                ...
                                mScrollbarAnimator = Utility.showScrollbar(mScrollbar, CameraHandler.this);
                            }
                        }
                        break;
    ...
    private OnSelectionListener onSelectionListener = new OnSelectionListener() {
        ...
        public void onClick(Img img, View view, int position) {
            if (LongSelection) {
                if (selectionList.contains(img)) {
                    ...
                    if (cameraOptions.getCount() <= selectionList.size()) {
                        Toast.makeText(CameraHandler.this,
        ...
        public void onLongClick(Img img, View view, int position) {
            if (cameraOptions.getCount() > 1) {
                Utility.vibe(CameraHandler.this, 50);
                LongSelection = true;
    ...
    public static void start(final Fragment context, final CameraOptions options) {
        PermUtil.checkForCamaraWritePermissions(context, new WorkFinish() {
            @Override
            public void onWorkFinish(Boolean check) {
                Intent i = new Intent(context.getActivity(), CameraHandler.class);
    ...
    public static void start(final FragmentActivity context, final CameraOptions options) {
        PermUtil.checkForCamaraWritePermissions(context, new WorkFinish() {
            @Override
            public void onWorkFinish(Boolean check) {
                Intent i = new Intent(context, CameraHandler.class);
                i.putExtra(OPTIONS, options);
                context.startActivityForResult(i, options.getRequestCode());
            }
        });
    ...
    private void initialize() {
        ...
        try {
            cameraOptions = (CameraOptions) getIntent().getSerializableExtra(OPTIONS);
        } catch (Exception e) {
            e.printStackTrace();
        }
        maxVideoDuration = cameraOptions.getVideoDurationLimitinSeconds() * 1000; //conversion in  milli seconds

        ((TextView) findViewById(R.id.message_bottom)).setText(cameraOptions.isExcludeVideos() ? R.string.pix_bottom_message_without_video : R.string.pix_bottom_message_with_video);
        status_bar_height = Utility.getStatusBarSizePort(CameraHandler.this);
        setRequestedOrientation(cameraOptions.getScreenOrientation());
        colorPrimaryDark =
                ResourcesCompat.getColor(getResources(), R.color.colorPrimaryPix, getTheme());
        camera = findViewById(R.id.camera_view);
        camera.setMode(Mode.PICTURE);
        if (cameraOptions.isExcludeVideos()) {
        ...
        camera.setLifecycleOwner(CameraHandler.this);

        if (cameraOptions.isFrontfacing()) {
        ...
        camera.addCameraListener(new CameraListener() {
            ...
            public void onPictureTaken(PictureResult result) {
                File dir = new File(Environment.getExternalStorageDirectory(), cameraOptions.getPath());
                if (!dir.exists()) {
                    dir.mkdirs();
                }
                ...
                result.toFile(photo, new FileCallback() {
                    ...
                    public void onFileReady(@Nullable File photo) {
                        ...
                        selectionList.add(img);
                        Utility.scanPhoto(CameraHandler.this, photo);
                        returnObjects();
                    }
                });
            }
            ...
            public void onVideoTaken(VideoResult result) {
                ...
                selectionList.add(img);
                Utility.scanPhoto(CameraHandler.this, result.getFile());
                camera.setMode(Mode.PICTURE);
                returnObjects();
            }
        ...
        selection_check.setVisibility((cameraOptions.getCount() > 1) ? View.VISIBLE : View.GONE);
        ...
        TOPBAR_HEIGHT = Utility.convertDpToPixel(56, CameraHandler.this);
        ...
        if ((cameraOptions.getPreSelectedUrls().size()) > cameraOptions.getCount()) {
            int large = cameraOptions.getPreSelectedUrls().size() - 1;
            int small = cameraOptions.getCount();
            for (int i = large; i > (small - 1); i--) {
                cameraOptions.getPreSelectedUrls().remove(i);
            }
        }
        DrawableCompat.setTint(selection_back.getDrawable(), colorPrimaryDark);
        updateImages();
    ...
    private void onClickMethods() {
        ...
        clickme.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (selectionList.size() >= cameraOptions.getCount()) {
                    Toast.makeText(CameraHandler.this,
                            String.format(getResources().getString(R.string.cannot_click_image_pix),
                                    "" + cameraOptions.getCount()), Toast.LENGTH_LONG).show();
                    return;
                }
        ...
        front.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                oa1.start();
                if (cameraOptions.isFrontfacing()) {
                    cameraOptions.setFrontfacing(false);
                    camera.setFacing(Facing.BACK);
                } else {
                    camera.setFacing(Facing.FRONT);
                    cameraOptions.setFrontfacing(true);
                }
            }
        });
    }
    ...
    private void updateImages() {
        ...
        Cursor cursor = Utility.getImageVideoCursor(CameraHandler.this, cameraOptions.isExcludeVideos());
        if (cursor == null) {
            return;
        }
        ...
        for (int i = 0; i < limit; i++) {
            ...
            String dateDifference = Utility.getDateDifference(CameraHandler.this, calendar);
            ...
            img.setPosition(pos);
            if (cameraOptions.getPreSelectedUrls().contains(img.getUrl())) {
        ...
        imageVideoFetcher = new ImageVideoFetcher(CameraHandler.this) {
            @Override
        ...
        imageVideoFetcher.setPreSelectedUrls(cameraOptions.getPreSelectedUrls());
        imageVideoFetcher.execute(Utility.getImageVideoCursor(CameraHandler.this, cameraOptions.isExcludeVideos()));
        cursor.close();
    ...
    private void setBottomSheetBehavior() {
        ...
        mBottomSheetBehavior.setBottomSheetCallback(new BottomSheetBehavior.BottomSheetCallback() {
            ...
            public void onSlide(@NonNull View bottomSheet, float slideOffset) {
                Utility.manipulateVisibility(CameraHandler.this, slideOffset, findViewById(R.id.arrow_up),
                        instantRecyclerView, recyclerView, status_bar_bg,
                        topbar, bottomButtons, sendButton, LongSelection);
                if (slideOffset == 1) {
                    Utility.showScrollbar(mScrollbar, CameraHandler.this);
...
//                cameraOptions.setPreSelectedUrls(new ArrayList<String>());
